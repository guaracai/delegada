document.addEventListener('DOMContentLoaded', function() {
  ...
  function addDayCells(year, month) {
    ...
    for (let day = ' '; day <= daysInMonth; day++) {
      ...
      // Adiciona a classe 'sunday' aos domingos
  ...
  // Adiciona evento do formulário ao mockEventDatabase
  document.getElementById('add-event-form').addEventListener('submit', function(e) {
    ...
    if (!mockEventDatabase[date]) {
      mockEventDatabase[date] = [];
    }
    mockEventDatabase[date].push({ title, description });
    generateCalendarDays(currentDate);
...
/*document.addEventListener('DOMContentLoaded', function() {
  ...
  function addDayCells(year, month) {
    ...
    for (let day = ' '; day <= daysInMonth; day++) {
      ...
      if (new Date(year, month, day).getDay() === 0) { // Se for domingo
        dayElement.classList.add('sunday');
      }
      ...
      if (new Date(year, month, day).getDay() === 0) { // 0 é Domingo
        dayElement.classList.add('sunday');
      }
      if (day === today.getDate() && month === today.getMonth() && year === today.getFullYear()) {
        dayElement.classList.add('current-day');
